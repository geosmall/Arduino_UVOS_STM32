...
void SysClkFullSpeed(void)
{
  ...
  /* Configure the main internal regulator output voltage.
     NOTE - Per datasheet, transition to Scale0 is only
            possible when in Scale1. */
  __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE1);
  while (!__HAL_PWR_GET_FLAG(PWR_FLAG_VOSRDY)) {}
...
void MPU_Config(void)
{
  ...
  */
  ...
  /* Prevent cache of DMA buffer regions, stored in SRAM3 (D2 domain)
     SRAM_D2c (xrw)  : ORIGIN = 0x30040000, LENGTH = 32K (SRAM3)    */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
